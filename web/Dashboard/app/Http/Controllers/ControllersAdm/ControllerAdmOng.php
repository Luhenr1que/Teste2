...
class ControllerAdmOng extends Controller
{
    public function situacaoNoticia(Request $request)
    {
        $ids = $request->input('noticiaSelecionadas');
        $acao = $request->input('acao');

        $status = ($acao == 'ativa') ? 'ativa' : 'negado';

        $noticia = NoticiaModel::find($ids);

        if ($status == 'negado') {
            $descricao = $request->input('motivo');


            NoticiaModel::whereIn('idNoticia', $ids)->update([
                'statusNoticia' => $status
            ]);

            return redirect()->action([AdminController::class, 'index'], ['opcao' => 2]);
        }
    ...
    public function dashboard()
    {
        ...
        return view('adm.pagina-inicial', compact('usuariosAtivos', 'usuariosInativos', 'noticiasAtivas', 'noticiasAnalise', 'usuarioQtaRefugiados', 'usuarioQtaNaoRefugiados', 'countryData', 'chartData', 'documentosStatus'));
    }

    public function trocarTema(Request $request)
    {
        try {
            $admin = Auth::guard('admin')->user();
            $admin->temaAdm = $request->tema;
            $admin->save();
